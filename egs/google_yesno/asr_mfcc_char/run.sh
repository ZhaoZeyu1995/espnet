...
recog_set="train_yyn train_ynn dev_yyn dev_ynn test_yyn test_ynn"

if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ### Task dependent. You have to make data the following preparation part by yourself.
    ### But you can utilize Kaldi recipes in most cases
    echo "stage 0: Data preparation"
    data_dir=/disk/scratch3/zzhao/data/google_command
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    echo "stage 1: Feature Generation"
     #Feature extraction
    for x in train_yyn dev_yyn test_yyn train_ynn dev_ynn test_ynn; do
        steps/make_mfcc.sh --nj 10 --write_utt2num_frames true data/${x} exp/make_mfcc/${x} ${mfccdir}
